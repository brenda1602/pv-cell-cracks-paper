...
\begin{itemize}
    ...
    \item Avoid combining SI and CGS units, such as current in amperes and
          magnetic
          field in oersteds. This often leads to confusion because equations do
          not
          balance dimensionally. If you must use mixed units, clearly state the
          units for
...
the equation. Use ``\eqref{eq}'', not ``Eq.~\eqref{eq}'' or ``equation
\eqref{eq}'', except at
...
\begin{itemize}
    ...
    \item In American English, commas, semicolons, periods, question and
          ...
          complete
          thought or name is cited, such as a title or full quotation. When
          quotation
          marks are used, instead of a bold or italic typeface, to highlight a
          word or
...
\paragraph{Positioning Figures and Tables} Place figures and tables at the top
and
